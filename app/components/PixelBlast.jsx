...
const createTouchTexture = (ownerDocument = document) => {
  const size = 64;
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      setSize();
      const ro = new ownerWindow.ResizeObserver(setSize);
      ro.observe(container);
      const randomFloat = () => {
        if (ownerWindow.crypto?.getRandomValues) {
          const u32 = new Uint32Array(1);
          ownerWindow.crypto.getRandomValues(u32);
      ...
      let liquidEffect;
      if (liquid) {
        touch = createTouchTexture(ownerDocument);
      ...
      // Listen on the whole hero instead of only the canvas. The foreground
      // content covers the canvas visually, so canvas-only listeners would
      // miss clicks on the title, avatar, and other transparent areas.
      const eventTarget = interactionTarget || container.closest('.hero-editorial') || renderer.domElement;
      ...
      const animate = timestamp => {
        if (autoPauseOffscreen && !visibilityRef.current.visible) {
          raf = ownerWindow.requestAnimationFrame(animate);
          return;
        }
